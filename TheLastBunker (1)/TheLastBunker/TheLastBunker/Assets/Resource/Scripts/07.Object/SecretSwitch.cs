...
public class SecretSwitch : Object
{
    public override void ActiveObj()
    {
        ...
        this.CanActive = true;
        bool pass = true;
        for (int i = 0; i < PlayInformation.Instance.PlayingRoom.secretswitchdown.Count; i++)
        {
            if (PlayInformation.Instance.PlayingRoom.secretswitchdown[i] == false)
            {
                pass = false;
            }
        }
        if (pass == true)
        {
            ...
            if (PlayInformation.Instance.PlayingRoom.LeftDoor != null)
            {
                if (PlayInformation.Instance.PlayingRoom.LeftDoor.CanUsingThis == false)
                {
                    ...
                    PlayInformation.Instance.PlayingRoom.w.RightDoor.CanActive = true;
                    PlayInformation.Instance.PlayingRoom.w.RightDoor.CanUsingThis = true;
                }
            }
        }
        else
