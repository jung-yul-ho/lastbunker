...
public class Room : MonoBehaviour
{
    ...
    //룸에서 사용되는 당기는 레버들
    public List<ClearRabber> clearrabeers;

    //룸에서 사용되는 시크릿 레버들
    public List<bool> secretswitchdown;
    public List<SecretSwitch> secretswitch;
            ...
            if(PlayInformation.Instance.PlayingRoom.objs[i] != null)
            {
                if (PlayInformation.Instance.PlayingRoom.objs[i].name == "레버(Clone)")
                {
                    if (PlayInformation.Instance.PlayingRoom.objs[i].GetComponent<Rabber>().LaserActive == false)
                    {
                        ...
                        {
                            if (PlayInformation.Instance.PlayingRoom.LaserControler[ii] != null)
                            {
